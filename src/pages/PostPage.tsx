...
function renderInlineLinks(text: string) {
  ...
  return parts.map((part, idx) => {
    if (/^https?:\/\//.test(part)) {
      return (
        <a key={`l-${idx}`} href={part} target="_blank" rel="noreferrer" className="underline decoration-gold/30 hover:decoration-gold break-all">
          {part}
        </a>
      );
    }
...
export default function PostPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center">
        <div className="font-mono text-[10px] uppercase tracking-widest text-ink/30 animate-pulse">
          Завантаження...
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-surface text-ink selection:bg-gold selection:text-ink">
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-[1000] border-b border-gold/20 bg-surface/90 backdrop-blur-md">
        <div className="grid grid-cols-2 md:grid-cols-4 px-4 md:px-8 py-3 md:py-4 text-[10px] md:text-xs font-mono uppercase tracking-widest items-center">
          <div className="col-span-1 flex items-center gap-2">
            <div className="w-4 h-4 bg-gold rounded-sm flex items-center justify-center">
              <div className="w-1.5 h-1.5 bg-surface rounded-sm animate-pulse" />
            </div>
            <Link to="/" className="font-bold tracking-tighter hover:text-gold-ink transition-colors">ОКО ГОРА</Link>
          </div>
          ...
          <div className="col-span-1 flex justify-end">
            <a href="https://t.me/oko_gora" target="_blank" rel="noreferrer"
              className="hover:text-gold-ink transition-colors flex items-center gap-1 font-bold">
      ...
      <main className="pt-24 md:pt-32 pb-24">
        ...
        <div className="px-4 md:px-8 mb-8 md:mb-12 max-w-[1000px] mx-auto">
          <Link
            to="/"
            className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-ink/40 hover:text-gold-ink transition-colors"
          >
        ...
        >
          {/* Meta */}
          <div className="flex flex-wrap items-center gap-4 mb-6 font-mono text-[9px] uppercase tracking-[0.2em] text-ink/40">
            <span className="font-bold text-ink/70">{post.id}</span>
          ...
          {/* Hero image */}
          {post.image && (
            <div className="w-full aspect-video overflow-hidden bg-surface-2 mb-10 md:mb-16 relative">
              <img
              ...
              {post.imageMeta?.qualityFlag && (
                <div className="absolute top-3 right-3 px-2 py-1 bg-ink/75 text-surface rounded-md font-mono text-[8px] tracking-widest uppercase">
                  {post.imageMeta.qualityFlag}
                </div>
              )}
            </div>
          ...
          <div className="max-w-2xl">
            <div className="space-y-5 text-lg md:text-xl leading-relaxed text-ink/75 mb-10">
              {splitParagraphs(post.text).map((paragraph, idx) => (
                /^[\-\u2022]\s/m.test(paragraph) ? (
                  <ul key={`${post.id}-p-${idx}`} className="space-y-2 pl-5 list-disc marker:text-gold/60">
                    {paragraph
                      .split('\n')
                      .map(line => line.trim())
                      .filter(Boolean)
            ...
            {post.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-8 border-t border-ink/10">
                <Tag className="w-3 h-3 text-ink/30 mt-0.5" />
                {post.tags.map(tag => (
                  <span
                    key={tag}
                    className="px-3 py-1 border border-ink/10 font-mono text-[9px] tracking-widest uppercase text-ink/50"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16">
              {related.map(p => (
                <Link key={p.id} to={`/post/${p.id}`} className="group">
                  {p.image && (
                    <div className="aspect-video w-full overflow-hidden bg-surface-2 mb-5">
                      <img
      ...
      <footer className="border-t border-gold/30 px-4 md:px-8 py-16 bg-surface-2 text-ink">
        <div className="max-w-[1000px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="font-mono text-[9px] text-ink/20 uppercase tracking-widest">
